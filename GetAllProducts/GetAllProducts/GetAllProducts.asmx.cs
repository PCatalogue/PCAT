...
namespace GetAllProducts
{
    ...
    public class GetAllProducts : System.Web.Services.WebService
    {
        ...
        public List<ProductClass> GetAllProductsList()
        {
            ...
            SqlDataAdapter adp = new SqlDataAdapter("select * from Products where PROD like 'Y'", con);
        ...
        {
            SqlConnection con = new SqlConnection("Persist Security Info=false;User ID=sa;Password=admin;Initial Catalog=Product_Catalogue;Data Source=192.168.0.153");
            SqlDataAdapter adp = new SqlDataAdapter("select * from products p inner join product_avail pav on p.product_id=pav.product_id inner join product_rates pra on p.product_id=pra.product_id where pav.service_id=@sid and pav.zipcode=@zip ", con);
